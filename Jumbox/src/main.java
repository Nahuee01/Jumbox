	...
	public static void main(String[] args) {
		...
		if (Login == null) {
			...
			if (Login.getRol().equalsIgnoreCase("gerente")) {
				...
				gerente nuevo = new gerente(Login.getNombre(), Login.getMail(), Login.getContrasena(), Login.getRol(),
						0);
				nuevo.menu();
			} else if (Login.getRol().equalsIgnoreCase("empleado")) {
				System.out.println("Es un empleado");
				empleado nuevo = new empleado(Login.getNombre(), Login.getMail(), Login.getContrasena(), Login.getRol(),
						0);
				nuevo.menu();
				...
				cliente nuevo = new cliente(Login.getNombre(), Login.getMail(), Login.getContrasena(), Login.getRol(),
						0);
				nuevo.menu();
			}
		}
	...
	public static Usuario Login(String mail, String contrasena) {
		...
		try {
			PreparedStatement statement = (PreparedStatement) con
					.prepareStatement("SELECT * FROM `usuario` WHERE `mail` = ? AND `contrasena` = ?");
			statement.setString(1, mail);
			statement.setString(2, contrasena);
			ResultSet resultSet = statement.executeQuery();
			if (resultSet.next()) {
				System.out.println("Rol recuperado: " + resultSet.getString("rol"));
				usuario = new Usuario(resultSet.getString("nombre"), resultSet.getString("mail"),

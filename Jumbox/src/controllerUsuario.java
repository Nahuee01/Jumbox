...
public class controllerUsuario {
	...
	public static ArrayList<Usuario> MostrarUsuarios() {
		...
		try {
			...
			while (resultSet.next()) {
				usuarios.add(new Usuario(resultSet.getString("nombre"), resultSet.getString("rol"),
						resultSet.getString("contrasena"), resultSet.getString("mail")));
			}
		} catch (Exception e) {
	...
	public static Usuario BuscarUsuario(int id) {
		...
		try {
			...
			ResultSet resultSet = statement.executeQuery();
			if (resultSet.next()) {
				nuevo = new Usuario(resultSet.getString("nombre"), resultSet.getString("mail"),
						resultSet.getString("contrasena"), resultSet.getString("rol"));
				nuevo.setIdUsuario(resultSet.getInt("idusuario"));
			} else {
	...
	public static void EliminarUsuario(int id) {
		try {
			...
			if (fila > 0) {
				usuarios.removeIf(u -> u.getIdUsuario() == id); // elimina el usuario por ID
